...
require_once WPFLICKR_PLUGIN_PATH . 'classes/cws-flickr-api.php';
 ...
 *
 */
class CWS_FlickrProApi extends CWS_FlickrApi{
...
*/
	...
	function get_recent_images_display( $size ) {
		...
		$cws_page = isset( $_GET['cws_page'] ) ? $_GET['cws_page'] : 1;	        
			...
			if( ( $response = $cache->load( 'photo_set_' . $id . '_' . $options['page'] . '_' . $options['per_page']) ) === false ) {
				...
				if( CWS_WPFlickr::is_pro_check() ) {				
					...
					$options['privacy_filter']	= ''; // empty this to return public and private
